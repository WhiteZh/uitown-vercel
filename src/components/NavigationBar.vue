...
<template>
  ...
  <div class="px-4 pt-4 flex flex-row justify-between items-end" id="navbar">
    ...
    <a href='#' class="text-sm self-center mx-1 py-1.5 px-3 rounded-full text-black font-bold bg-[#1ac8db]" @click.prevent="openLogin = !openLogin" v-if="!user">Join in the Town</a>
    ...
    <RouterLink :to="user === undefined ? {} : {name: 'create'}" class="text-sm self-center mx-1 py-1.5 px-3 rounded-full text-black font-bold bg-[#99dfec]" @click="user === undefined ? notifications.push({message:'Please login before creating new styles', color: 'yellow'}) : null">Create</RouterLink>
  </div>
